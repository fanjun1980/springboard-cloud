...
public class DateUtils {
	...
	public static ArrayList<String> getDate(String starDate, String endDate){
		...
		for(int i=0;;i++){
			...
			if(!dates.contains(temp)) dates.add(temp);
			if(temp.equals(startDate))break;
		}
		return dates;
	}
	...
	public static String dateStrForDB(String date,String format) {
		...
		{
			String re = "";
			if(date == null || date.toLowerCase().equals("null")) return date;
			
			format = format.toLowerCase().replace("hh:mm", "hh24:mi");
			re = "to_date('" + date + "','" + format + "')";
			return re;
		}
		else
		{
    ...
    public static boolean isWeekEnd(Date date){
    	Date end = DateUtils.getLastDayOfTheWeek(date);
    	return org.apache.commons.lang3.time.DateUtils.isSameDay(end, date);
    }
    ...
    public static boolean isMonthEnd(Date date){
    	Date end = DateUtils.getLastDayOfTheMonth(date);
    	return org.apache.commons.lang3.time.DateUtils.isSameDay(end, date);
    }
}