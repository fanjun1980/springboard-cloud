...
import io.springboard.framework.utils.encode.JaxbMapper;
...
public class JaxbMapperTest {
	...
	public void xmlToObject() throws JAXBException {
		...
		assertTrue(user.getInterests().contains("movie"));
	}
	 ...
	 */
	@Test
	public void toXmlWithListAsRoot() throws JAXBException {
		User user1 = new User();
		user1.setId(1L);
	...
	private static void assertXmlByDom4j(String xml) {
		...
		assertEquals(user.attribute("id").getValue(),"1");

		Element interests = (Element) doc.selectSingleNode("/user/interests");
		assertEquals(interests.elements().size(), 2);
		assertEquals(((Element) interests.elements().get(0)).getText(),"movie");
	}
